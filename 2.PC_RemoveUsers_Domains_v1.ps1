...
Import-Csv -Path $PCDomains_Csv | 
 ForEach-Object { `
    ...
    Write-Host "The following users are now on the recycle bin: `n #To delete all users from Recycle: `n Get-MsolUser -ReturnDeletedUsers | Remove-MsolUser -RemoveFromRecycleBin -Force"
